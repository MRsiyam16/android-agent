...
report. Everything explanatory is in `android-agent/README.md` and `android-agent/docs/`.
...
**The `test_*.py` files in `android-agent/scratch/`** are gitignored one-off drivers pointed
...
`android-agent/SYSTEM_MEMORY.md` is generated by `system_memory.py` at the end of every run
...
`android-agent/` has a GitHub remote (`MRsiyam16/android-agent`); the repo root is also a
git repo. Commit and push after a completed feature, not per file edit. Tag milestones
...
- `android-agent/README.md` — the full guide
- `android-agent/docs/ARCHITECTURE.md` — state hashing and exploration algorithms
- `android-agent/docs/SETUP.md`, `docs/EXAMPLES.md`
