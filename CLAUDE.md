...
| Android emulator | `emulator.py` (AVD lifecycle over `config.ADB_PATH`; `python emulator.py --ensure`) |
| The Bugmaster bridge | `verifications.py` (the verdict log), `backend/routes/verifications.py`, `start_verifier.py` (the second instance) — see `docs/VERIFIER.md` |
...
`start_verifier.py` runs a **second copy of this harness** on port 8001 with its own notebook
(`PROJECTS_DIR=verify-projects`), answering fix-verification jobs from Bugmaster's pipeline —
see `docs/VERIFIER.md`. The separation is not tidiness: the build it tests is a patch that is
deployed nowhere, so a `bug` there means "the fix did not work", and letting one onto the
product board would file a defect against the shipped app. Verification runs therefore **never
file Blackcode issues**, which the manager's prompt says and `report_verification` enforces.
It refuses to start while the QA Master on :8000 holds a device lock — one phone, one driver,
and a lock cannot be seen across a port.
...
- `docs/VERIFIER.md` — the second instance, and the device bridge it answers for Bugmaster
